...
case basic_shape::cube:
    ...
    for(int i=0;i<8;i++)vc[i]=color_rgb(0.4,0,0);
    for(int i=8;i<16;i++)vc[i]=color_rgb(0,0.4,0);
    for(int i=16;i<24;i++)vc[i]=color_rgb(0,0,0.4);
...
case basic_shape::triangle:
    name = std::string("basic_triangle");
    n_vertices=3;
    n_triangles=1;
    t = new utype3 [n_triangles];
    v = new vector3 [n_vertices];
    ...
    v[1] = vector3(1,0,0);
    v[2] = vector3(0,0,1);
    vn[0]=vn[1]=vn[2]=vector3(0,-1,0);
    vc[0]=color_rgb(1,0,0);vc[1]=color_rgb(0,1,0);vc[2]=color_rgb(0,0,1);
    for(int i=0;i<n_vertices;i++)vn[i]+=v[i];
    t[0]={0,1,2};
break;

case basic_shape::square:
    name = std::string("basic_square");
    n_vertices=4;
    n_triangles=2;
    t = new utype3 [n_triangles];
    v = new vector3 [n_vertices];
    vn = new vector3 [n_vertices];
    vc = new color_rgb [n_vertices];
    v[0] = vector3(0.5,0.5,0);
    v[1] = vector3(-0.5,0.5,0);
    v[2] = vector3(-0.5,-0.5,0);
    v[3] = vector3(0.5,-0.5,0);
    vn[0]=vn[1]=vn[2]=vn[3]=vector3(0,0,1);
    vc[0]=color_rgb(1,0,0);vc[1]=color_rgb(0,1,0);vc[2]=color_rgb(0,0,1);vc[3]=color_rgb(1,1,1);
    for(int i=0;i<n_vertices;i++)vc[i]=color_rgb(0);
    for(int i=0;i<n_vertices;i++)vn[i]+=v[i];
    t[0]={0,1,2};t[1]={2,3,0};
break;
