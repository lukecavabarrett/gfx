...
camera::camera(utype xres, utype yres) : forward(), inverse(), near(20), far(50000), h(0.48*yres/xres), w(0.48), yres(yres), xres(xres), video_buffer(new int [yres*xres]) {}
...
camera& camera::rotate_z_about_origin(dtype theta){
    
    dtype px=forward.value[3][0], py=forward.value[3][1], pz=forward.value[3][2];
    
    forward.push_back(transformator::translate(-px,-py,-pz));
    forward.push_back(transformator::rotate_z(theta));
    forward.push_back(transformator::translate(px,py,pz));
    
    inverse.push_front(transformator::translate(-px,-py,-pz));
    inverse.push_front(transformator::rotate_z(-theta));
    inverse.push_front(transformator::translate(px,py,pz));
    
    return *this;
}

camera& camera::rotate_x_about_origin(dtype theta){
...
camera& camera::rotate_y_about_origin(dtype theta){
